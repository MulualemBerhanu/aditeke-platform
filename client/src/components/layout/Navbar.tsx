...
const Navbar = () => {
  ...
  // Function to handle header reveal on mouse enter
  const handleRevealHeader = () => {
    if (isHidden) {
  ...
  // Function to reset reveal trigger when mouse leaves the header
  const handleHideRevealTrigger = () => {
    setIsRevealTriggered(false);
  };
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    // Only hide when scrolling down more than 50px to avoid accidental triggers
    const scrollDifference = Math.abs(latest - lastScrollY.current);
    
    // Scrolling down - hide header
    if (latest > lastScrollY.current && !isHidden && scrollDifference > 20) {
      setIsHidden(true);
    } 
    // Scrolling up - show header
    // We make this more sensitive so it appears with slight upward scroll
    else if (latest < lastScrollY.current && isHidden && scrollDifference > 5) {
      setIsHidden(false);
    }
  ...
  return (
    <>
      {/* Header reveal trigger area - only shown when header is hidden */}
      {isHidden && !isRevealTriggered && (
        <div 
          className="fixed top-0 w-full h-5 z-50 bg-transparent"
          onMouseEnter={handleRevealHeader}
        />
      )}
      ...
      <motion.header
        ...
        onMouseLeave={handleHideRevealTrigger}
      >
