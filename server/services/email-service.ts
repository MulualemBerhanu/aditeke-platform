...
// Import the email wrapper which handles both real sending and fallbacks
import { sendEmail as sendWrappedEmail } from '../utils/emailWrapper';
...
export async function sendWelcomeEmail(params: {
  ...
  try {
    ...
    console.log(`Preparing welcome email for ${username} (${email})`);
    
    // Check if the BREVO_API_KEY is set
    const brevoApiKey = process.env.BREVO_API_KEY;
    if (!brevoApiKey) {
      console.error('BREVO_API_KEY is not set in environment variables for welcome email');
      return false;
    }
    ...
    // Convert to plain text for better deliverability and accessibility
    const textContent = `
      Hello ${name},
    ...
    // Use the email wrapper to send
    const result = await sendWrappedEmail({
      to: email,
    ...
    console.log(`Welcome email to ${email} result:`, result);
    return true;
  } catch (error: any) {
...
export async function sendPasswordResetEmail(params: {
  ...
  try {
    ...
    console.log(`Preparing password reset email for ${username} (${email})`);
    ...
    console.log(`Password reset email to ${email} result:`, result);
    return true;
  } catch (error: any) {
